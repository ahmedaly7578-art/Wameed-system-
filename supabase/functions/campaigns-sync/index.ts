...
Deno.serve(async (req) => {
  ...
  const db = supa();
  const { start, endInclusive } = lastCompletedWeek();
  ...
  else q = q.in("status", ["connected", "error"]); // retry errored ones on the daily run too
  ...
  for (const acc of accounts) {
    try {
      ...
      await db.from("ad_accounts").update({ status: "connected", last_synced_at: new Date().toISOString() }).eq("id", acc.id);
      results.push({ id: acc.id, platform: acc.platform, ok: true, hadData: !!numbers });
    } catch (e) {
      await db.from("ad_accounts").update({ status: "error" }).eq("id", acc.id);
      results.push({ id: acc.id, platform: acc.platform, ok: false, error: String(e instanceof Error ? e.message : e) });
    }
  }

  return json({ ok: true, weekStart, synced: results.filter(r => r.ok).length, failed: results.filter(r => !r.ok).length, results });
});
